...
director_rev_map = joblib.load("director_target_encoding.pkl")
...
def preprocess_new_movie(movie_dict: dict) -> pd.DataFrame:
    ...
    try:
        row['director_mean_log_revenue'] = director_rev_map[director]
    except:
        row['director_mean_log_revenue'] = np.mean(list(director_rev_map))
...
def predict_new_movie(movie_dict: dict, pipeline) -> dict:
    ...
    return {
        ...
        "log_revenue_predicted": log_pred
    }